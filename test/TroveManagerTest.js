...
contract('TroveManager', async accounts => {
  ...
  it("liquidate(): decreases ActivePool ETH and rDebt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    const activePool_RDebt_Before = (await activePool.rDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(activePool_RDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1ETH:100R, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    /* close Bob's Trove. Should liquidate his ether and R,
    leaving Alice’s ether and R debt in the ActivePool. */
    ...
    const activePool_RDebt_After = (await activePool.rDebt()).toString()

    assert.equal(activePool_ETH_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePool_RDebt_After, A_totalDebt)
  })

  it("liquidate(): increases DefaultPool ETH and R debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    const defaultPool_RDebt_Before = (await defaultPool.rDebt()).toString()

    assert.equal(defaultPool_ETH_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPool_RDebt_Before, '0')
    ...
    const defaultPool_RDebt_After = (await defaultPool.rDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(defaultPool_RDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // price drops to 1ETH:100R, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    // close Bob's Trove.  His ether*0.995 and R should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    await deploymentHelper.mintR(rToken, owner);
    wstETHTokenMock.approve(activePool.address, dec(100, 'ether'), { from: bob})
    await openTrove({ ICR: toBN(dec(200, 18)), amount: dec(100, 'ether'), extraParams: { from: bob } })

    // Alice creates a single trove with 0.7 ETH and a debt of 70 R
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    ...
    await openTrove({ ICR: toBN(dec(218, 16)), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(201, 16)), extraParams: { from: defaulter_4 } })
  ...
  it("liquidate(): Pool offsets increase the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(202, 16)), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(201, 16)), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(219, 16)), extraParams: { from: defaulter_4 } })
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { rAmount: A_RAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraRAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { rAmount: B_RAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraRAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { rAmount: C_RAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraRAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeRDebt_0 = await activePool.rDebt()
    const defaultRDebt_0 = await defaultPool.rDebt()

    await troveManager.liquidate(alice)
    const activeRDebt_A = await activePool.rDebt()
    const defaultRDebt_A = await defaultPool.rDebt()

    await troveManager.liquidate(bob)
    const activeRDebt_B = await activePool.rDebt()
    const defaultRDebt_B = await defaultPool.rDebt()
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await deploymentHelper.mintR(rToken, owner);
    await openTrove({ ICR: toBN(dec(8, 18)), extraRAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraRAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraRAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    const defaultPoolRDebt = await defaultPool.rDebt()
    ...
    assert.isAtMost(th.getDifference(pendingRDebt_C, defaultPoolRDebt), 1000)
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    await openTrove({ ICR: toBN(dec(10, 18)), extraRAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(209, 16)), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(216, 16)), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(201, 16)), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: defaulter_4 } })
    ...
    assert.equal((await sortedTroves.getSize()).toString(), '8')
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    const { totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(201, 16)), extraParams: { from: alice } })
    const { totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(212, 16)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2000, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(1800, 16)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    const { totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(202, 16)), extraParams: { from: alice } })
    const { totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(195, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2000, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(1800, 16)), extraParams: { from: erin } })
    ...
    // Liquidate - trove C in between the ones to be liquidated!
    const liquidationArray = [alice, carol, bob, dennis, erin]
    await troveManager.batchLiquidateTroves(liquidationArray, { from: whale });
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraRAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('redeemCollateral(): cancels the provided R with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraRAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraRAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraRAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraRAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    /* check that Dennis' redeemed 20 R has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraRAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraRAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraRAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraRAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraRAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST ---

    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 R), 20000%
    const { rAmount: F_RAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraRAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraRAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraRAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraRAmount: dec(20, 18), extraParams: { from: carol } })
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraRAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraRAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraRAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 R from Carol's Trove
    ...
    // Since Alice already redeemed 1 R from Carol's Trove, Dennis was  able to redeem:
    //  - 9 R from Carol's
    //  - 8 R from Bob's
    // for a total of 17 R.

    // Dennis calculated his hint for redeeming 2 R from Alice's Trove, but after Alice's transaction
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveRAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraRAmount: amount, extraParams: { from: bob } })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { rAmount: B_RAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraRAmount: A_debt, extraParams: { from: bob } })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraRAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraRAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500R to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(500, 18), extraParams: { from: alice } })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraRAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraRAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
  ...
  it("redeemCollateral(): caller can redeem their entire RToken balance", async () => {
    ...
    // Alice opens trove and transfers 400 R to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraRAmount: dec(400, 18), extraParams: { from: alice } })
    ...
    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraRAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraRAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraRAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    // Check activePool debt reduced by  400 R
    const activePool_debt_after = await activePool.rDebt()
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed R (assuming 1 R has value of $1)", async () => {
    ...
    // Alice opens trove and transfers 1000 R each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(4990, 18), extraParams: { from: alice } })
    ...
    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraRAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraRAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraRAmount: dec(1090, 18), extraParams: { from: dennis } })
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    ...
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraRAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraRAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.rDebt()).toString(), totalDebt)
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraRAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraRAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraRAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraRAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check total R supply
    const activeR = await activePool.rDebt()
    const defaultR = await defaultPool.rDebt()

    const totalRSupply = activeR.add(defaultR)
    th.assertIsApproximatelyEqual(totalRSupply, totalDebt)
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraRAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraRAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraRAmount: dec(100, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraRAmount: dec(100, 18), extraParams: { from: D } })
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraRAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraRAmount: dec(100, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraRAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraRAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraRAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraRAmount: dec(100, 18), extraParams: { from: D } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { rAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(1, 24), extraParams: { from: alice } })
  ...
  it("getPendingRDebtReward(): Returns 0 if there is no pending rDebt reward", async () => {
    await deploymentHelper.mintR(contracts.rToken, whale);
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(201, 16)), extraRAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraRAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraRAmount: totalDebt, extraParams: { from: whale } })

    // Price drops
    await priceFeed.setPrice(dec(100, 18))

    await troveManager.liquidate(defaulter_1, { from: whale })

    // Confirm defaulter_1 liquidated
    assert.isFalse(await sortedTroves.contains(defaulter_1))

    // Confirm there are no pending rewards from liquidation
    const current_L_RDebt = await troveManager.L_RDebt()
    assert.equal(current_L_RDebt, 0)

    const carolSnapshot_L_RDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_RDebt, 0)

    const carol_PendingRDebtReward = await troveManager.getPendingRDebtReward(carol)
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraRAmount: B_totalDebt, extraParams: { from: A } })
