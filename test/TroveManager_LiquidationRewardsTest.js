...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    const carol_Coll = ((await troveManager.Troves(carol))[1]
      .add(await troveManager.getPendingCollateralTokenReward(carol)))
      .toString()
  ...
  it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
    ...
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingCollateralTokenReward(bob)))
      .toString()
    const dennis_Coll = ((await troveManager.Troves(dennis))[1]
      .add(await troveManager.getPendingCollateralTokenReward(dennis)))
      .toString()
    const erin_Coll = ((await troveManager.Troves(erin))[1]
      .add(await troveManager.getPendingCollateralTokenReward(erin)))
      .toString()
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
    ...
    const freddy_ETHReward = (await troveManager.getPendingCollateralTokenReward(freddy)).toString()
  ...
  it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100000, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100000, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100000, 18), extraParams: { from: C } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(20000, 16)), extraRAmount: dec(10, 18), extraParams: { from: D } })
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100000, 18), extraParams: { from: E } })
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const B_ETHGain_1 = await troveManager.getPendingCollateralTokenReward(B)
    const C_ETHGain_1 = await troveManager.getPendingCollateralTokenReward(C)
    const D_ETHGain_1 = await troveManager.getPendingCollateralTokenReward(D)
    const E_ETHGain_1 = await troveManager.getPendingCollateralTokenReward(E)
    ...
    const B_ETHGain_2 = await troveManager.getPendingCollateralTokenReward(B)
    const D_ETHGain_2 = await troveManager.getPendingCollateralTokenReward(D)
    const E_ETHGain_2 = await troveManager.getPendingCollateralTokenReward(E)
    ...
    const B_ETHGain_3 = await troveManager.getPendingCollateralTokenReward(B)
    const D_ETHGain_3 = await troveManager.getPendingCollateralTokenReward(D)
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(110, 18), extraParams: { from: carol } })
    ...
    const bob_RDebt = ((await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingRDebtReward(bob)))
      .toString()
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    */
    ...
    const alice_RDebt = ((await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingRDebtReward(alice)))
      .toString()
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // A, B, C, D open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraRAmount: dec(110, 18), extraParams: { from: bob } })
    wstETHTokenMock.approve(activePool.address, _998_Ether, { from: carol})
    const { collateral: C_coll } = await openTrove({ extraRAmount: dec(110, 18), amount: _998_Ether, extraParams: { from: carol } })
    wstETHTokenMock.approve(activePool.address, dec(1000, 'ether'), { from: carol})
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(110, 18), amount: dec(1000, 'ether'), extraParams: { from: dennis } })
    ...
    const alice_ETHReward_1 = await troveManager.getPendingCollateralTokenReward(alice)
    const bob_ETHReward_1 = await troveManager.getPendingCollateralTokenReward(bob)
    const carol_ETHReward_1 = await troveManager.getPendingCollateralTokenReward(carol)
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    totalColl: 3.49 ETH
    totalDebt 380 R (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingCollateralTokenReward(bob)))
      .toString()
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    // A, B, C open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(100, 18), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingCollateralTokenReward(bob), B_pendingRewardsAfterL1), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingCollateralTokenReward(carol), C_pendingRewardsAfterL1), 1000000)
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingCollateralTokenReward(carol), C_pendingRewardsAfterL2), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingCollateralTokenReward(dennis), D_pendingRewardsAfterL2), 1000000)
    ...
    // E and F open troves
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(110, 18), extraParams: { from: erin } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraRAmount: dec(110, 18), extraParams: { from: freddy } })
    ...
    const carol_pendingETHReward = (await troveManager.getPendingCollateralTokenReward(carol)).toString()

    const dennis_rawColl = (await troveManager.Troves(dennis))[1].toString()
    const dennis_pendingETHReward = (await troveManager.getPendingCollateralTokenReward(dennis)).toString()

    const erin_rawColl = (await troveManager.Troves(erin))[1].toString()
    const erin_pendingETHReward = (await troveManager.getPendingCollateralTokenReward(erin)).toString()
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ extraRAmount: dec(100, 18), amount: toBN(dec(35, 15)), extraParams: { from: dennis } })
    ...
    const D_pendingRewardsAfterL2 = D_coll.mul(th.applyLiquidationFee(B_collAfterL1)).div(C_collAfterL1.add(D_coll))
    const C_collAfterL2 = C_collAfterL1.add(C_pendingRewardsAfterL2)
    assert.isAtMost(th.getDifference(await troveManager.getPendingCollateralTokenReward(carol), C_pendingRewardsAfterL2), 10000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingCollateralTokenReward(dennis), D_pendingRewardsAfterL2), 10000000)
    ...
    */
    ...
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ extraRAmount: dec(100, 18), amount: toBN(dec(1, 22)), extraParams: { from: erin } })
    wstETHTokenMock.approve(activePool.address, toBN('700000000000000'), { from: erin})
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ extraRAmount: dec(100, 18), amount: toBN('700000000000000'), extraParams: { from: freddy } })
