...
contract('PositionManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const denominatorColl_1 = (await positionManager.getEntireSystemColl()).sub(A_entireColl_0)
    ...
    // Check accumulated ETH gain for each position
    const B_ETHGain_1 = await positionManager.getPendingCollateralTokenReward(B)
    const C_ETHGain_1 = await positionManager.getPendingCollateralTokenReward(C)
    const D_ETHGain_1 = await positionManager.getPendingCollateralTokenReward(D)
    const E_ETHGain_1 = await positionManager.getPendingCollateralTokenReward(E)

    // Check gains are what we'd expect from a distribution proportional to each position's entire coll
    ...
    await positionManager.addColl(B, B, dec(1, 'ether'), { from: B })
    await priceFeed.setPrice(dec(100, 18))
    ...
    const denominatorColl_2 = (await positionManager.getEntireSystemColl()).sub(C_entireColl_1)
    ...
    const B_ETHGain_2 = await positionManager.getPendingCollateralTokenReward(B)
    const D_ETHGain_2 = await positionManager.getPendingCollateralTokenReward(D)
    const E_ETHGain_2 = await positionManager.getPendingCollateralTokenReward(E)
    ...
    const denominatorColl_3 = (await positionManager.getEntireSystemColl()).sub(E_entireColl_2)
    ...
    const B_ETHGain_3 = await positionManager.getPendingCollateralTokenReward(B)
    const D_ETHGain_3 = await positionManager.getPendingCollateralTokenReward(D)
