...
contract('BorrowerOperations', async accounts => {
  ...
  beforeEach(async () => {
    ...
    it("withdrawLUSD(): reverts if max fee > 100%", async () => {
      ...
      await assertRevert(borrowerOperations.withdrawLUSD(dec(2, 18), dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0% and 100%")
      await assertRevert(borrowerOperations.withdrawLUSD('1000000000000000001', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0% and 100%")
    })
    ...
    it("withdrawLUSD(): borrowing at non-zero base rate sends LUSD fee to fee recipient", async () => {
      ...
      const feeRecipient_LUSDBalance_Before = await lusdToken.balanceOf(lqtyStaking.address)
    ...
    it("withdrawLUSD(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // Check fee recipient's LUSD balance has increased
      const feeRecipient_LUSDBalance_After = await lusdToken.balanceOf(feeRecipient)
      assert.isTrue(feeRecipient_LUSDBalance_After.gt(feeRecipient_LUSDBalance_Before))
    ...
    it("repayLUSD(): reverts when borrowing rate = 0% and it would leave trove with net debt < minimum net debt", async () => {
      // Check if borrowing rate is 0
      const borrowingRate = await troveManager.getBorrowingRate()
      assert.isTrue(borrowingRate.eq(toBN(0)))

      // Make the LUSD request 1 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await wstETHTokenMock.approve(activePool.address, dec(100, 30), { from: A})
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
      ...
      await wstETHTokenMock.approve(activePool.address, collTopUp, { from: alice})
      await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, LUSDRepayment, false, alice, alice, collTopUp, { from: alice }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.adjustTrove(th._100pct, 0, LUSDRequest_D, true, D, D, 0, { from: D })
    ...
    it("adjustTrove(): Borrowing at zero rate doesn't change LUSD balance of LQTY staking contract", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("adjustTrove(): Borrowing at non-zero rate changes LUSD balance of fee recipient", async () => {
      const feeRecipient = await borrowerOperations.feeRecipient()
      ...
      // Check staking LUSD balance before > 0
      const feeRecipient_LUSDBalance_Before = await lusdToken.balanceOf(feeRecipient)
      assert.isTrue(feeRecipient_LUSDBalance_Before.gt(toBN('0')))
    ...
    it("adjustTrove(): Borrowing at zero rate doesn't change LQTY staking contract LUSD fees-per-unit-staked", async () => {
      await wstETHTokenMock.approve(activePool.address, toBN(dec(100, 'ether')), { from: whale})
      await openTrove({ extraLUSDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), amount: toBN(dec(100, 'ether')), extraParams: { from: whale } })
    ...
    it.skip("adjustTrove(): Borrowing at non-zero rate changes LQTY staking contract LUSD fees-per-unit-staked", async () => {
      await troveManager.setBorrowingSpread(dec(5, 15), { from: owner })
    ...
    it("adjustTrove(): Reverts if borrower has insufficient LUSD balance to cover his debt repayment", async () => {
      ...
      const repayLUSDPromise_B = borrowerOperations.adjustTrove(th._100pct, 0, bobDebt, false, B, B, 0, { from: B })
    ...
    it("closeTrove(): succeeds when borrower's LUSD balance is equals to his entire debt and borrowing rate = 0%", async () => {
      ...
      assert.isTrue(B_troveDebt.sub(B_LUSDBal).eq(LUSD_GAS_COMPENSATION))

      const closeTroveB = await borrowerOperations.closeTrove({ from: B })
      assert.isTrue(closeTroveB.receipt.status)
    })

    it("closeTrove(): reverts if borrower has insufficient LUSD balance to repay his entire debt when borrowing rate > 0%", async () => {
      await troveManager.setBorrowingSpread(dec(5, 15), { from: owner })
      ...
      await assertRevert(closeTrovePromise_B, "BorrowerOps: Caller doesn't have enough LUSD to make repayment")
    })
    ...
    it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
      ...
      const openTroveTx = await borrowerOperations.openTrove(th._100pct, D_LUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, dec(200, 'ether'), { from: D })
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, LUSDRequest_D, D, D, dec(500, 'ether'), { from: D })
    ...
    it.skip("openTrove(): Borrowing at zero rate doesn't change the LQTY staking contract LUSD fees-per-unit-staked", async () => {
    ...
    it.skip("openTrove(): Borrowing at non-zero rate changes the LQTY staking contract LUSD fees-per-unit-staked", async () => {
      await troveManager.setBorrowingSpread(dec(1, 15), { from: owner })
    ...
    it("openTrove(): Borrowing at zero base rate charges no fee", async () => {
      ...
      const txC = await borrowerOperations.openTrove(th._100pct, LUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, dec(100, 'ether'), { from: C })
      const _LUSDFee = toBN(th.getEventArgByName(txC, "LUSDBorrowingFeePaid", "_LUSDFee"))

      assert.isTrue(_LUSDFee.eq(toBN(0)))
    })
    ...
    it("openTrove(): increases the Trove's LUSD debt by the correct amount", async () => {
      ...
      await wstETHTokenMock.approve(activePool.address, dec(100, 'ether'), { from: alice })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), alice, alice, dec(100, 'ether'), { from: alice })
