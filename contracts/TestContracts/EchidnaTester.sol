...
contract EchidnaTester {
    ...
    uint256 private MCR;
    uint256 private constant CCR = 1500000000000000000; // 150% TODO: delete when doing https://github.com/tempusfinance/raft/issues/17
    ...
    function getAdjustedETH(uint256 actorBalance, uint256 _ETH, uint256 ratio) internal view returns (uint256) {
        uint256 price = priceFeedTestnet.getPrice();
        require(price > 0);
        uint256 minETH = ratio * LUSD_GAS_COMPENSATION / price;
        require(actorBalance > minETH);
        uint256 ETH = minETH + _ETH % (actorBalance - minETH);
        return ETH;
    }

    function getAdjustedLUSD(uint256 ETH, uint256 _LUSDAmount, uint256 ratio) internal view returns (uint256) {
        uint256 price = priceFeedTestnet.getPrice();
        uint256 LUSDAmount = _LUSDAmount;
        uint256 compositeDebt = LUSDAmount + LUSD_GAS_COMPENSATION;
    ...
    function openTroveExt(uint256 _i, uint256 _ETH, uint256 _LUSDAmount) public payable {
        uint256 actor = _i % NUMBER_OF_ACTORS;
    ...
    function withdrawLUSDExt(uint256 _i, uint256 _amount, address _upperHint, address _lowerHint, uint256 _maxFee)
        external
    {
        uint256 actor = _i % NUMBER_OF_ACTORS;
    ...
    function repayLUSDExt(uint256 _i, uint256 _amount, address _upperHint, address _lowerHint) external {
        uint256 actor = _i % NUMBER_OF_ACTORS;
    ...
    function adjustTroveRawExt(
        ...
        echidnaProxies[actor].adjustTrovePrx(
            _ETH, _collWithdrawal, _debtChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFee
        );
    }
