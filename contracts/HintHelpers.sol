...
contract HintHelpers is LiquityBase, Ownable, CheckContract {
        ...
        returns (address firstRedemptionHint, uint256 partialRedemptionHintNICR, uint256 truncatedLUSDamount)
    {
        ...
        while (currentTroveuser != address(0) && remainingLUSD > 0 && _maxIterations-- > 0) {
            uint256 netLUSDDebt = _getNetDebt(troveManager.getTroveDebt(currentTroveuser))
                + troveManager.getPendingLUSDDebtReward(currentTroveuser);

            if (netLUSDDebt > remainingLUSD) {
                if (netLUSDDebt > MIN_NET_DEBT) {
                    uint256 maxRedeemableLUSD = LiquityMath._min(remainingLUSD, netLUSDDebt - MIN_NET_DEBT);

                    uint256 ETH =
                        troveManager.getTroveColl(currentTroveuser) + troveManager.getPendingETHReward(currentTroveuser);

                    uint256 newColl = ETH - maxRedeemableLUSD * DECIMAL_PRECISION / _price;
                    uint256 newDebt = netLUSDDebt - maxRedeemableLUSD;
